...
const mapPresenca = r => ({
  ...
  FuncionarioId: r.funcionarioid, FuncionarioNome: r.funcionarionome, Funcao: r.funcao, Diaria: r.diaria, Transporte: r.transporte, Alimentacao: r.alimentacao,
  DiariaPaga: r.diariapaga, CustoTotal: r.custototal, ObraId: r.obraid, ObraNome: r.obranome, ObraEndereco: r.obraendereco
});
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.match(/^\/presencas\/[^/]+\/autorizar-hora-extra$/)) {
      const id = url.split('/')[2];
